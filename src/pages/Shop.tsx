    ...
    <section className="section page">
      ...
      <div className="flex gap-2 overflow-x-auto pb-2.5 mb-7">
        {['الكل', ...occasions].map(c => (
          <button key={c} onClick={() => setCat(c)} className={`px-5 py-2.5 rounded-full text-[12.5px] font-bold whitespace-nowrap border transition-all ${cat === c ? 'bg-ink text-surface-alt border-ink' : 'bg-surface border-border text-muted hover:border-primary hover:text-primary'}`} type="button">{c}</button>
        ))}
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-20 text-muted">
          <h3 className="text-ink font-bold text-lg mb-2">مفيش نتائج</h3>
          <p className="text-sm">جرّب كلمة تانية أو غيّر القسم</p>
        </div>
      ) : (
